...
L'extraction d'informations du \tname{} est la plus complexe. La difficulté tient au fait que cette balise peut contenir des informations structurées de façon très différente, ce qui demande de détecter des motifs à différents degrés et à différentes étapes. Dans un premier temps, les prénoms sont extraits. Ils sont identifiés à l'aide de plusieurs \glspl{expression régulière} qui permettent d'identifier des prénoms complets et abrégés, qu'ils soient composés ou non, en prenant en compte les différents types d'abréviations possibles (un prénom composé peut être entièrement abrégé ou seulement un prénoms peut être abrégé) et les différentes typographies (séparer les prénoms avec des traits d'union ou non, par exemple). Dans le cas où un prénom serait abrégé, il est si possible reconstruit: des initiales sont remplacées par un nom complet. Cela permet d'augmenter le taux de bons identifiants \wkd{}, mais vient avec plusieurs difficultés techniques, comme nous le verrons. Une fois ce nom extrait, le type d'information contenue dans le \tname{} doit être identifié: en fonction du type d'information (géographique, historique, nominative...), différents traitements sont mis en place. Cette identification se fait par détection de motifs augmentée de listes contenant du vocabulaire spécifique et de \glspl{table de conversion} et par un processus éliminatoire. Pour commencer, le script identifie si le \tname{} correspond à \enquote{Le même} ou \enquote{La même}. Dans ce cas, l'entrée est la même que la précédente et le dictionnaire produit pour la précédente entrée est réutilisé. Ensuite, il détecte et élimine les entrées qui ne contiennent pas d'informations spécifiques (\enquote{Documents divers}); si c'est le cas, il n'extrait pas d'informations du \tname{}. Les listes et les tables utilisées à cette étape contiennent des informations géographiques (nom d'anciennes provinces françaises\footnote{\ref{appendix:convprov}}, de départements du \scl{XIX}\footnote{\ref{appendix:convdpt}}, d'anciennes colonies\footnote{\ref{appendix:convcolonie}} et de pays\footcite{\ref{appendix:convpays}}) et historiques (contenant des évènements importants\footnote{\ref{appendix:convevt}}). Si une information géographique est repérée, alors des informations normalisées grâce à des tables de conversions sont ajoutées au dictionnaire; sinon, le script cherche à identifier des évènements historiques et, de même, ajoute leur équivalent normalisé au dictionnaire. Par processus d'élimination, si aucune de ces informations n'a été détectée, alors le contenu du \tname{} est le nom d'une personne.
...
\begin{figure}[!p]
	...
	\tikz[node distance=1cm, scale=0.70, transform shape]{
		...
		\node[transf]%
			(nopartr) at (-5, -19)%
			{Nom sans parenthèses: nettoyage simple des données \\ Réinjection du prénom extrait au début dans le \texttt{fname} et du reste dans le \texttt{fname}};	
		\node[transf]%
			(partr) at (1, -19)%
