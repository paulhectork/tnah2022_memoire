%%%%%%%%%%%% métadonnées %%%%%%%%%%%%
\author{Paul, Hector KERVEGAN}
\title{Modélisation, enrichissement sémantique et diffusion d'un corpus textuel semi-structuré: le cas des catalogues de vente de manuscrits}
\date{15.09.2022}

%%%%%%%%%%%% generic packages: encodage, format, langue, police, math %%%%%%%%%%%%
\documentclass[a4paper,12pt,twoside]{book}
\usepackage[english, french]{babel}
\usepackage{fontspec}
\usepackage{lmodern}
\usepackage{graphicx}
\usepackage{enumitem}
\usepackage{lscape}
\usepackage{array}
\usepackage{multicol}
\usepackage{amsmath}
\usepackage{diagbox}
\usepackage{afterpage}
\usepackage{rotating}
\usepackage{changepage}


%%%%%%%%%%%% code %%%%%%%%%%%%
\usepackage[newfloat]{minted}
\SetupFloatingEnvironment{listing}{name=Données brutes}
\usemintedstyle{borland}
\setminted{linenos, breaklines, tabsize=4, bgcolor=lightmauve}
\usepackage{listings}
\lstset{%
	basicstyle=\footnotesize\ttfamily,%
	numbers=left,%
	backgroundcolor=\color{lightmauve},%
	breaklines=true%
}
% multi-page-minted
\usepackage{caption}
\newenvironment{multipageminted}{\captionsetup{type=listing}}{}


%%%%%%%%%%%% graphiques et couleurs %%%%%%%%%%%%
\usepackage{tikz}
\usetikzlibrary{shapes.geometric}

% thx https://latexcolor.com/
\definecolor{palepink}{rgb}{0.98, 0.85, 0.87}
\definecolor{plum}{rgb}{0.44, 0.11, 0.11}
\definecolor{lilac}{rgb}{0.8, 0.8, 1.0}
\definecolor{peachpuff}{rgb}{1.0, 0.85, 0.73}
\definecolor{lightpink}{rgb}{1.0, 0.71, 0.76}
\definecolor{pearl}{rgb}{0.94, 0.92, 0.84}
\definecolor{lightmauve}{rgb}{0.86, 0.82, 1.0}
\definecolor{cherryblossompink}{rgb}{1.0, 0.72, 0.77} 	
\definecolor{nadeshikopink}{rgb}{0.96, 0.68, 0.78}
\definecolor{pastelviolet}{rgb}{0.8, 0.6, 0.79}
\definecolor{moccasin}{rgb}{0.98, 0.92, 0.84}
\definecolor{persimmon}{rgb}{0.93, 0.35, 0.0}
\definecolor{electricpurple}{rgb}{0.75, 0.0, 1.0}
\definecolor{wildstrawberry}{rgb}{1.0, 0.26, 0.64}
\definecolor{outrageousorange}{rgb}{1.0, 0.43, 0.29}
\definecolor{darkterracotta}{rgb}{0.8, 0.31, 0.36}


% db interaction: db
% main step (start, stop...): main
% transfo/relation: transf
\tikzstyle{base} = [%
	draw,%
	rectangle,%
	rounded corners=3pt,%
	minimum height=1cm,%
	fill=lilac,%
	draw=plum,%
	text centered,%
	text width=4cm%
]
\tikzstyle{db} = [%
	cylinder,%
	shape border rotate=90,%
	aspect=0.25,%
	fill=pastelviolet,%
	draw=plum,%
	text centered,%
	text width=4cm%
]
\tikzstyle{transf} = [%
	trapezium,%
	trapezium left angle=70,%
	trapezium right angle=110,%
	minimum height=1cm,%
	fill=lightpink,%
	draw=plum,%
	text centered,%
	text width=3cm%
]
\tikzstyle{choice} = [%
	diamond,%
	minimum width=2cm,%
	aspect=2,%
	fill=peachpuff,%
	draw=plum,%
	text centered,%
	text width=4cm%
]
\tikzstyle{expl} = [%
	rectangle,%
	draw=plum,%
	fill=moccasin,%
	text centered,%
	text width=4cm%
]
\tikzstyle{outline} = [%
	rectangle,%
	rounded corners,%
	draw=plum,%
	text width=4cm,%
	ultra thick
]
\tikzstyle{outline-no-width} = [%
	rectangle,%
	rounded corners,%
	draw=plum,%
	ultra thick
]
\tikzstyle{blank} = [%
	rectangle,%
	draw=white,%
	draw opacity=0%
]
\tikzstyle{arrow} = [%
	thick,%
	->,%
	>=stealth,%
	color=plum%	
]
\tikzstyle{doublearrow} = [%
	thick,%
	<->,%
	>=stealth,%
	color=plum,%
]
\tikzstyle{line} = [%
	thick,%
	color=plum%	
]
\tikzstyle{dotted} = [%
	thick,%
	->,%
	>=stealth,%
	color=plum,%
	dash pattern=on 3pt off 4pt on \the\pgflinewidth off 4pt%
]
\pgfdeclarelayer{bg}    % declare background layer
\pgfsetlayers{bg,main}  % set the order of the layers (main is the standard layer)

% images
\usepackage{subcaption}

% mise en page ENC
\usepackage[margin=2.5cm]{geometry}
\usepackage{setspace}
\onehalfspacing
\setlength\parindent{1cm}

% bibliographie
\usepackage[toc]{appendix}
\usepackage{csquotes}
\usepackage[backend=biber,sorting=nyt,style=enc]{biblatex}
\nocite{*}
\addbibresource{bibliographie/katabase.bib}
\addbibresource{bibliographie/ecostat.bib}
\addbibresource{bibliographie/textprocessing.bib}
\addbibresource{bibliographie/webgeneralites.bib}
\addbibresource{bibliographie/webapi.bib}
\addbibresource{bibliographie/websemantic.bib}

\DeclareSourcemap{%
 \maps[datatype=bibtex]{%
 	\map[overwrite]{%
 		\perdatasource{bibliographie/katabase.bib}
 		\step[fieldset=keywords, fieldvalue={,katabase}, append]
 	}
	\map[overwrite]{%
		\perdatasource{bibliographie/textprocessing.bib}
		\step[fieldset=keywords, fieldvalue={,text}, append]
	}
 	\map[overwrite]{%
 		\perdatasource{bibliographie/ecostat.bib}
 		\step[fieldset=keywords, fieldvalue={,econometrie}, append]
 	}
 	\map[overwrite]{%
 		\perdatasource{bibliographie/webgeneralites.bib}
 		\step[fieldset=keywords, fieldvalue={,webgeneralites}, append]
 	}
	\map[overwrite]{%
		\perdatasource{bibliographie/websemantic.bib}
		\step[fieldset=keywords, fieldvalue={,websemantic}, append]
	}
	\map[overwrite]{%
		\perdatasource{bibliographie/webapi.bib}
		\step[fieldset=keywords, fieldvalue={,webapi}, append]
	}
  }
}


% notes de bas de page, table des matières citations et index
\usepackage{imakeidx}
% \makeindex
% \makeindex[name=lieux,title=Index des noms de lieux]
\usepackage{tocbibind}

% autres paquets
\usepackage{ifthen}
 
% hyperref
\usepackage{hyperref}
\hypersetup{
	colorlinks=false,
	citebordercolor=outrageousorange,
	filebordercolor=electricpurple,
	linkbordercolor=electricpurple,
	menubordercolor=darkterracotta,
	urlbordercolor=wildstrawberry,
	runbordercolor=wildstrawberry,
	% linkcolor=electricviolet,
	% filecolor=violet,
	% citecolor=electricviolet,      
	% urlcolor=electricviolet,
	pdfauthor={Paul, Hector KERVEGAN}, 
	pdftitle={Modélisation, enrichissement sémantique et diffusion d'un corpus textuel semi-structuré: le cas des catalogues de vente de manuscrits}, 
	pdfsubject={Traitement de données textuelles},
	pdfkeywords={catalogues de vente}{mss}{katabase}{ocr}{web sémantique}{linked open data}{web de données}{traitement automatisé du language}{détection de motifs}{api}{fair}{rest}
}


% glossaire et acronmymes
\usepackage[automake, acronym, toc]{glossaries}
\setacronymstyle{short-long}
\renewcommand{\glsnamefont}[1]{\makefirstuc{#1}}  % capitalize 1st letter in \printglossaries
\makeglossaries
\newglossaryentry{apig}{
	name={API},
	description={Une interface de programmation d'application (\textit{Application Programming Interface} en anglais) est un protocole qui pemret à un programme de communiquer avec un autre programme. Ce protocole documenté, correspond à un ensemble d'opérations permettant à un programme \enquote{consommateur} d'utiliser des fonctionnalités d'un programme \enquote{fournisseur}, comme par exemple de récupérer ou d'envoyer des données au fournisseur.}
}
\newglossaryentry{api}{
	type=\acronymtype,
	name={API},
	description={Application Programming Interface},
	first={\textit{Application Programming Interface} (\enquote{Interface de programmation d'application}, ou API) \glsadd{apig}},
	see=[Glossaire:]{apig}
}
\newglossaryentry{graph}{
	name={base de données en graphe},
	plural={bases de données en graphe},
	description={Une base de données en graphe sert à stocker des données reliées entre elles au sein d'un graphe ou d'un réseau. Les données correspondent aux nœuds du graphe et les relations sont représentées sous forme d'arrêtes. Souvent, données et relations disposent d'identifiants uniques définis grâce à des ontologies. L'interaction avec de telles bases de données est permise grâce à des languages de requêtes, comme \sparql{}. La représentation en graphe a pour but de mettre l'accent sur l'interrelation entre les données et de permettre des liens complexes entre elles.\footcite{noauthor_graph_2022}}
}
\newglossaryentry{bleu}{
	name={score BLEU},
	plural={scores BLEU},
	description={Un score BLEU (bilingual evaluation understudy) est un score servant à évaluer la qualité de la traduction d'un texte par une machine. Il est obtenu en comparant le texte traduit par une machine avec un plusieurs textes de référence (c'est-à-dire, des traductions faites par des humains du même texte).}
}
\newglossaryentry{cts}{
	type=\acronymtype,
	name={CTS},
	description={\textit{Canonical Text Services}},
	first={\textit{Canonical Text Services} (CTS) \glsadd{ctsg}},
	see=[Glossaire:]{ctsg}
}
\newglossaryentry{ctsg}{
	name={CTS},
	description={\textit{Canonical Text Services} (CTS) est une API et une spécification pour identifier et diffuser des citations au sein de textes encodés. L'API spécifie la manière d'identifier un texte, un témoin, un passage ou encore une série de textes à l'aide d'un URN (un identifiant unique analogue aux URI). Le CTS est une API, mais ne spécifie pas comment une API CTS doit être implémentée: ce qui importe, c'est que l'API CTS permette d'accéder à des extraits de textes grâce à leurs citations\footcite{smith_four_2012}.}
}
\newglossaryentry{dts}{
	type=\acronymtype,
	name={DTS},
	description={\textit{Distributed Text Services}},
	first={\textit{Distributed Text Services} (DTS) \glsadd{dtsg}},
	see=[Glossaire:]{dtsg}
}
\newglossaryentry{dtsg}{
name={DTS},
description={Comme le CTS, le \textit{Distributed Text Services} (DTS) est une API visant à identifier et permettre l'identification et la diffusion de texte dans une architecture client-serveur. Cette API, développée à partir de 2018 sur un modèle communautaire, demande une réponse en XML-TEI et des métadonnées encodées en JSON-LD; ces spécifications techniques la distinguent de CTS, qui ne repose pas sur un format particulier. Comme le CTS, DTS ne définit pas comment implémenter l'API\footcite{almas_distributed_2021}.}
}
\newglossaryentry{dictionnaire}{
	name={dictionnaire},
	plural={dictionnaires},
	description={Un dictionnaire est un format de données structuré en python qui associe à une donnée -- dite clé -- une ou plusieurs données nommées valeurs.}
}
\newglossaryentry{expression régulière}{
	name={expression régulière},
	plural={expressions régulières},
	description={Une expression régulière, ou expression rationnelle est une chaîne de caractère, écrite selon une syntaxe précise, qui permet de détecter des motifs dans du texte\footcite{noauthor_expression_2022}. Les expressions régulières s'appuient sur la classification des caractères en classes (minuscules, majuscules, chiffres, espaces), sur des structures alternatives (un caractère ou un autre) et exclusives (un caractère n'ayant pas certaines propriétés) pour repérer des motifs. Par exemple, \enquote{2022} correspond au motif: \texttt{\textbackslash{}d{4}}, soit \enquote{quatre chiffres à la suite}. Une adresse mail est également un motif: \texttt{[\^~(@|\textbackslash{}s)]+@[\^~(@|\textbackslash{}s)]+}, soit \enquote{plusieurs caractères qui ne sont ni un espace une arobase, suivi d'une arobase, suivi de plusieurs caractères qui n'est ni un espace ni une arobase}.}
}
\newglossaryentry{frbr}{
	type=\acronymtype,
	name={FRBR},
	description={\textit{Functional Requirements for Bibliographic Records}},
	first={\textit{Functional Requirements for Bibliographic Records} (\enquote{Besoins fonctionnels pour notices bibliographiques}, ou FRBR)}
}
\newglossaryentry{fair}{
	type=\acronymtype,
	name={FAIR},
	description={\textit{Findable Accessible Interoperable Reusable}},
	first={\textit{Findable Accessible Interoperable Reusable} (FAIR) \glsadd{fairg}},
	see=[Glossaire:]{fairg}
}
\newglossaryentry{fairg}{
	name={FAIR},
	description={Les principes \textit{Findable Accessible Interoperable Reusable} (FAIR) correspondent à un ensemble de préconisations simples pour le partage de données dans un contexte de sciences ouvertes\footcite{boeckhout_fair_2018}.}
}
\newglossaryentry{http}{
	type=\acronymtype,
	name={HTTP},
	description={\textit{Hypertext Transfer Protocol}},
	first={\textit{HyperText Transfer Protocol} (HTTP) \glsadd{httpg}},
	see=[Glossaire:]{httpg}
}
\newglossaryentry{httpg}{
	name={HTTP},
	description={Le \textit{Hypertext Transfer Protocol} (HTTP, ou \enquote{protocole de transfert hypertexte}), est un protocole qui permet la communication entre différentes machines dans une architecture client-serveur, comme celle du Web. C'est donc le protocole qui est à la base d'Internet, puisqu'il établit la manière dont des machines peuvent interagir au sein d'un réseau. Il définit un ensemble de méthodes pour interagir avec le serveur -- demander (\texttt{GET}) des données, en envoyer (\texttt{POST})... Il définit également la manière dont le serveur répond au client et un ensemble d'erreurs\footcite[p. 116-127]{fielding_architectural_2000}, chacune disposant d'un code pour l'identifier (l'erreur 404 indiquant que la ressource demandée n'existe pas, ou l'erreur 418 signifiant \enquote{Je suis une théière}, un canular qui pouve l'humour des concepteur.ice.s du Web).}
}
\newglossaryentry{log}{
	name={fichier de log},
	plural={fichiers de log},
	description={Un fichier de log est un fichier qui est créé pendant l'exécution d'un algorithme pour sauvegarder des informations qui le concernent. Il s'agit en général d'informations annexes sur l'exécution du script: messages d'erreur, données produites en cours d'utilisation...}
}
\newglossaryentry{nel}{
	type=\acronymtype,
	name={NEL},
	description={Named Entity Linking (NEL)},
	first={\textit{Named Entity Linking} (NEL) \glsadd{nelg}},
	see=[Glossaire:]{nelg}
}
\newglossaryentry{nelg}{
	name={Named Entity Linking (NEL)},
	description={Le \textit{Named Entity Linking}, traduit en français par \enquote{résolution d'entités nommées} ou \enquote{liage d'entités nommées} consiste à lier des entités -- c'est-à-dire des objets uniques, comme des lieux, des personnes ou des organisations -- à des sources de connaissance externes, comme des bases de connaissance en ligne.}
}
\newglossaryentry{oaipmh}{
	type=\acronymtype,
	name={OAI-PMH},
	description={\textit{Open Archives Initiative Protocol for Metadata Harversting}},
	first={\textit{Open Archives Initiative Protocol for Metadata Harversting} (OAI-PMH) \glsadd{oaipmhg}},
	see=[Glossaire:]{oaipmhg}
}
\newglossaryentry{oaipmhg}{
	name={OAI-PMH},
	description={L'OAI-PMH (\textit{Open Archives Initiative Protocol for Metadata Harversting}) est un protocole notamment utilisé par les bibliothèques et les archives; il permet de partager des métadonnées sur une collection de documents. Les données sont partagées en \xml{} et s'appuient fortement sur l'usage du \textit{Dublin Core}, un standard simple pour la structuration de métadonnées. Contrairement au CTS et au DTS, l'API OAI-PMH ne permet pas de partager un document, mais seulement ses métadonnées; comme ces deux autres standards cependant, le standard OAI-PMH ne définit pas une implémentation\footcite{prime-claverie_defi_2017}.}
}
\newglossaryentry{ontologie}{
	name={ontologie},
	description={Une ontologie correspond à la définition d'un ensemble de catégories, de propriétés et de relations qui unissent des données et des concepts; cet ensemble est complété par une modélisation (souvent sous forme graphique), qui indique la relation entre les différents termes de l'ontologie\footcite{noauthor_ontology_2022}. Ceux-ci sont souvent liés de façon hiérarchique (plusieurs termes spécifiques pouvant être dérivés d'un terme générique). Dans le Web sémantique, chaque terme a lui-même un identifiant unique, ce qui garantit une implémentation uniforme pour l'ontologie et réduit le risque de présence de doublons.}
}
\newglossaryentry{open source}{
	name={\textit{open source}},
	description={L'\textit{open source}, ou \enquote{code ouvert} définit à la fois un statut légal et un mouvement social. L'\textit{open source} correspond à un logicel ou programme dont le code est librement consultable en ligne et réutilisable selon différentes licences plus ou moins permissives, comme la \textit{GNU GPL v3.0} ou la \textit{MIT License}. Ce statut légal a beaucoup gagné en popularité, en partie pour des raisons pragmatiques (un code ouvert est maintenu gratuitement, ce qui diminue les coûts d'entreprises) et symboliques (les GAFAM se sont beaucoup associées à ce courant en raison de l'image positive qu'il donne). Le statut est également associé à plusieurs mouvements en faveur du partage et de la réutilisation des connaissances, tel que la science ouverte. Certaines franges de l'\textit{open source} sont plus radicales et ont une conception ouvertement politique de l'accessibilité au code, accessibilité qui permettrait aux utilisateur.ice.s de regagner en autonomie, de développer de nouveaux modèles communautaires et de reprendre le contrôle sur leurs comportements en ligne et leur consommation médiatique. Parmi les représentant.e.s les plus proéminent.e.s de ce courant se trouve Richard Stallman, créateur de la \textit{GNU Foundation} et du système d'exploitation GNU, à la base de la majorité des systèmes d'exploitation utilisant Linux.}
}
\newglossaryentry{ocr}{
	type=\acronymtype,
	name={OCR},
	description={Reconnaissance optique de caractères (OCR)},
	first={Reconnaissance optique de caractères (OCR)}
}
\newglossaryentry{océrisation}{
	name={\textit{océrisation}},
	description={Ce terme est dérivé de l'acronyme OCR (\textit{optical character recognition}, ou \enquote{reconnaissance optique de caractères}). Il permet de désigner le processus de reconnaissance du texte dans un document numérisé.}
}
\newglossaryentry{odd}{
	type=\acronymtype,
	name={ODD},
	description={\textit{One document does it all} \enquote{Un document pour tout faire} (ODD)},
	first={\textit{One document does it all} \enquote{Un document pour tout faire} (ODD)}
}
\newglossaryentry{ohco}{
	type=\acronymtype,
	name={OHCO},
	description={\textit{One ordered collection of objects} (OHCO)},
	first={\textit{One ordered collection of objects} (\enquote{Une collection ordonnée d'objets}, OHCO)}
}
\newglossaryentry{precision}{
	name={précision},
	plural={précisions},
	description={La précision est, avec le rappel, une des deux mesures statistiques permettant de calculer un score F1. La précision mesure la validité des résultats obtenus. Elle correspond à la proportion d'items pertinents par rapport à l'ensemble des items proposés, soit la proportion de vrais positifs parmi tous les résultats positifs obtenus. Elle se calcule de la façon suivante:
		\begin{displaymath}
			P = \frac{\text{Nombre de résultats pertinents obtenus (vrais positifs)}}{\text{Ensemble des résultats obtenus (vrais positifs + faux positifs)}}
		\end{displaymath}
	}
}
\newglossaryentry{python}{
	name={python},
	description={Python est un language de programmation impérative (c'est-à-dire, basé sur la production et la transformation de données par une suite d'instructions) créé en 1991. Il est particulièrement utilisé dans le domaine du traitement de données et des humanités numériques. C'est le langage le plus utilisé dans le projet \mssktb{}}
}
\newglossaryentry{rappel}{
	name={rappel},
	plural={rappels},
	description={En statistiques, le rappel est l'une des deux mesures permettant de calculer un score F1. Le rappel mesure la quatité de résultats pertinents obtenus. Il correspond à la proportion de résultats pertinents obtenus par rapport à tous les résultats pertinents qui auraient dû être obtenus. Il se calcule donc de la façon suivante:
		\begin{displaymath}
			R = \frac{\text{Nombre de résultats pertinents obtenus (vrais positifs)}}{\text{Ensemble des résultats pertinents (vrais positifs + faux négatifs)}}
		\end{displaymath}
	}
}
\newglossaryentry{rest}{
	type=\acronymtype,
	name={REST},
	description={\textit{Representational state transfer}},
	first={\textit{Representational state transfer} (REST) \glsadd{restg}},
	see=[Glossaire:]{restg}
}
\newglossaryentry{restg}{
	name={REST},
	description={L'architecture REST, définie par Roy Fielding (2000)\footcite{fielding_architectural_2000} un standard pour le design et le fonctionnement interne d'API. Contrairement à d'autres standards (DTS, CTS, OAI-PMH), elle ne définit pas la manière de cibler des données grâce à des identifiants; cette architecture définit plutôt comment l'interaction client-serveur doit être implémentée, à l'aide d'une série de principes de design. C'est le standard le plus répandu pour la conception d'API, bien qu'il soit souvent mal implémenté, ce qui a ammené au développement d'API \textit{REST-like}.}
}
\newglossaryentry{score F1}{
	name={score F1},
	plural={scores F1},
	description={Le score F1, ou \textit{F-score}, est la moyenne harmonique de la précision (vrais positifs par rapport au total de résultats obtenus) et du rappel (nombre de résultats positifs par rapport au total de résultats positifs).\footcite{noauthor_precision_2022, noauthor_f-score_2022}. Le score F1 a l'avantage de prendre en compte les vrais et les faux positifs. Ce score, dont la valeur est contenue entre 0 et 1, permet de mesurer l'exactitude d'un algorithme d'apprentissage machine, ou d'un moteur de recherche. Soit \(F\) le score F1, \(P\) la précision et \(R\) le rappel, sa formule est la suivante:
	\begin{align*}	
		P &= \frac{\text{items pertinents obtenus}}{\text{total des items obtenus}} \\~\\
		R &= \frac{\text{items pertinents obtenus}}{\text{ensemble des items pertinents}} \\~\\
		F &= 2 \times \frac{P \times R}{P + R}
	\end{align*}	
	}
}
\newglossaryentry{SPARQL}{
	name={SPARQL},
	description={SPARQL est un language de requête qui permet d'interagir avec une base de données au format RDF. \enquote{SPARQL exprime des requêtes sur des sources de données diverses [...]. SPARQL rend possible la requête de données en graphes [...], avec des données conjointes et disjointes. SPARQL permet également l'agrégation de données, les sous-requêtes, la négation, la création de données à l'aide d'expression, le test de données et la contrainte des requêtes. Les résultats de requêtes sparql peuvent être des jeux de résultats ou des graphes RDF.}\footnote{\enquote{SPARQL can be used to express queries across diverse data sources [...]. SPARQL contains capabilities for querying [...] graph patterns along with their conjunctions and disjunctions. SPARQL also supports aggregation, subqueries, negation, creating values by expressions, extensible value testing, and constraining queries by source RDF graph. The results of SPARQL queries can be result sets or RDF graphs.}\cite{harris_sparql_2013} (traduction de l'auteur).}}
}
\newglossaryentry{table de conversion}{
	name={table de conversion},
	plural={tables de conversion},
	description={Une table de conversion est, tout simplement, un dictionnaire qui contient en clés un certain nombre d'informations telles qu'elles figurent dans le texte et, en valeurs, une version normalisée de cette information. Cela permet de détectier des motifs à extraire autant de normaliser les informations.}
}
\newglossaryentry{tei}{
	name={TEI},
	description={La \texttt{TEI} (acronyme de \textit{Text Encoding Initiative}, \enquote{Initiative pour l'encodage du texte}), est un projet universitaire visant à standardiser l'encodage du texte. La \texttt{TEI} définit un ensemble d'éléments avec une sémantique précise qui définissent différentes parties d'un texte: paragraphes, notes de bas de pages... La \texttt{TEI} se caractérise par une approche philologique du texte qui permet d'encoder des documents très différents: il est par exemple possible de constituer des éditions critiques ou d'encoder des manuscrits.}
}
\newglossaryentry{uri}{
	type=\acronymtype,
	name={URI},
	description={\textit{Uniform Resource Identifier}},
	first={\textit{Uniform Resource Identifier} (\enquote{Identifiant unique de ressource}, ou URI) \glsadd{urig}},
	see=[Glossaire:]{urig}
}
\newglossaryentry{urig}{
	name={URI},
	description={Une URI (\textit{Uniform Resource Identifier}) est une chaîne de caractères qui identifie une ressoource sur un réseau. Cette URI est permanente, et permettra d'identifier une ressource même si celle-ci est supprimée. L'URL est un type particulier d'URI qui, en plus de d"crire une resource, la représente aussi: un URL de page \textit{Wikipedia} permet d'identifier une page de façon unique, comme toute URI (aucune autre page sur internet n'ayant le même URL); mais en plus, elle pêermet d'accéder à la page elle-même.}
}
\newglossaryentry{url}{
	type=\acronymtype,
	name={URL},
	description={\textit{Uniform Resource Locator}},
	first={\textit{Uniform Resource Locator} (URL) \glsadd{urlg}},
	see=[Glossaire:]{urlg}
}
\newglossaryentry{urlg}{
	name={URL},
	description={Une URL (\textit{Uniform Resource Locator}) est un identifiant unique sous la forme d'une chaîne de caractères pointant vers une ressource sur le Web. De la même manière qu'une cote de bibliothèque pointe vers un livre précis, un URL pointe vers une page ou une donnée sur le Web. L'URL permet d'agir sur une ressource (en envoyant des données...) et peut être associée à des données ainsi qu'à des reésentations (une page Web, par exemple).}
}
\newglossaryentry{xml}{
	name={XML},
	description={Le \texttt{XML} (acronyme d'\textit{eXtensible Markup Language}, ou \enquote{Langage de balisage extensible}) est un métalangage de balisage informatique. Il définit une syntaxe simple pour encoder des données textuelles à l'intérieur d'éléments. Ces éléments contiennent du texte entre une balise ouvrante (par exemple: \texttt{<note>}) et une balise fermante (\texttt{</note>}). On parle de métalangage car le \texttt{XML} définit une syntaxe pour l'encodage, mais pas un vocabulaire: différents standards existent qui définissent un ensemble de balises \texttt{XML}, comme la \texttt{TEI}.}
}
\newglossaryentry{w3c}{
	type=\acronymtype,
	name={\textit{World Wide Web Consortium}},
	description={\textit{World Wide Web Consortium}},
	first={\textit{World Wide Web Consortium (W3C)}}
}

% modèle: \newacronym{dom}{\textsc{dom}}{\emph{Document Object Model}}
% modèle bis: \newglossaryentry{meta}{name=métadonnée,description={donnée servant à définir ou décrire une autre donné quel que soit son support}}

% commandes perso
% pour les abbréviations: technologies en mono, sigles et sites web en italique
\newcommand{\cleartwopages}{%
	\afterpage{
		\pagestyle{empty}
		\newpage~\newpage~\newpage~
	}
}
\newcommand{\widepage}{\newgeometry{left=1cm,right=1cm}}  % smaller margins; use \restoregeometry after
\makeatletter
\newcommand{\scl}[1]{%
	% normalisation des siècles en écriture romaine
	% #1 : le siècle en chiffres romains
	#1%
	\ifthenelse{\equal{#1}{I}}{\up{er}}{\up{ème}}%
	~s%
	\@ifnextchar.{}{.\@ifnextchar,{}{ }}
}
\newcommand{\api}{\gls{api}}
\newcommand{\alto}{\texttt{Alto}}
\newcommand{\escr}{\textit{eScriptorium}}
\newcommand{\html}{\texttt{HTML}}
\newcommand{\js}{\texttt{Javascript}}
\newcommand{\json}{\texttt{JSON}}
\newcommand{\ktb}{\textit{Katabase}}
\newcommand{\mss}{\textit{MSS}}
\newcommand{\mssktb}{\mss{} / \ktb{}}
\newcommand{\py}{\texttt{\gls{python}}}
\newcommand{\rgxsg}{\gls{expression régulière}}
\newcommand{\rgxpl}{\glspl{expression régulière}}
\newcommand{\rdf}{\texttt{RDF}}
\newcommand{\sparql}{\texttt{\gls{SPARQL}}}
\newcommand{\tei}{\texttt{\gls{tei}}}
\newcommand{\tsv}{\texttt{TSV}}

\newcommand{\titem}{\texttt{tei:item}}
\newcommand{\tname}{\texttt{tei:name}}
\newcommand{\tnum}{\texttt{tei:num}}
\newcommand{\ttrait}{\texttt{tei:trait}}
\newcommand{\tp}{\texttt{tei:p}}
\newcommand{\tdesc}{\texttt{tei:desc}}
\newcommand{\tdate}{\texttt{tei:date}}
\newcommand{\tmeasure}{\texttt{tei:measure}}
\newcommand{\tnote}{\texttt{tei:note}}

\newcommand{\xml}{\texttt{\gls{xml}}}
\newcommand{\xmltei}{\texttt{XML-TEI}}
\newcommand{\xsl}{\texttt{XSL}}
\newcommand{\wkd}{\textit{Wikidata}}

% modèle: https://www.overleaf.com/project/61a6185ee83481070ab68d99
% liste des mémoires: https://docs.google.com/spreadsheets/d/1CZRaEX4BMqHNtqr97Dhg9UV9bPTt_4XS_IgPL5A-ZbU/edit

%%%%%%%%%%%%%%%%%%%%%%%%%%%% DÉBUT %%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{document}
	\onehalfspacing
	
	\begin{titlepage}
		\begin{center}		
			\bigskip
			
			\begin{large}
				ÉCOLE NATIONALE DES CHARTES\\
			\end{large}
			
			\rule{2cm}{0.02cm}
			
			\bigskip
			
			\begin{Large}
				\textbf{Paul, Hector Kervegan}
			\end{Large}
		
			\bigskip
		
			\begin{normalsize} 
				\textit{licencié ès histoire de l'art}\\
				\textit{diplômé de master en histoire de l'art et humanités numériques}
			\end{normalsize}
			
			\vfill
			
			\textbf{%
				\Huge{M}%
				\Large{%
					\MakeUppercase{%
						odélisation, enrichissement sémantique et diffusion %
						d'un corpus textuel semi-structuré: le cas des catalogues %
						de vente de manuscrits
					}
				}
			}
			
			\vfill
			
			\begin{large}
				Mémoire pour le diplôme de master \\
				\emph{Technologies numériques appliquées à l'histoire}
				
				\bigskip
				
				2022
			\end{large}
			
		\end{center}
	\end{titlepage}


%%%%%%%%%%%%%%%%%%%% RÉSUMÉ + CORPS DU TEXTE %%%%%%%%%%%%%%%%%%%%

\input{front.tex}

\input{part_i.tex}

\input{part_ii.tex}

\input{part_iii.tex}

\input{conclusion.tex}


%%%%%%%%%%%%%%%%%%%%%% ANNEXES %%%%%%%%%%%%%%%%%%%%%%
\cleartwopages
\part*{Annexes}
\addcontentsline{toc}{part}{Annexes}
\input{appendix.tex}	


%%%%%%%%%%%%%%%%%%%%%% BIBLIOGRAPHIE %%%%%%%%%%%%%%%%%%%%%%
\backmatter
\cleartwopages
\chapter*{Bibliographie}
\chaptermark{Bibliographie}
\addcontentsline{toc}{chapter}{Bibliographie}
\printbibliography[heading=subbibintoc,keyword={katabase},title={Projet \mssktb{}}]
\printbibliography[heading=subbibintoc,keyword={text},title={Édition numérique, traitement automatisé et analyse de texte}]
\printbibliography[heading=subbibintoc, keyword={webgeneralites}, title={Technologies du web: généralités}]
\printbibliography[heading=subbibintoc, keyword={websemantic}, title={Technologies du web: Web sémantique}]
\printbibliography[heading=subbibintoc, keyword={webapi}, title={Technologies du web: API}]
\printbibliography[heading=subbibintoc,keyword={econometrie},title={Histoire et marché de l'art, économétrie et statistiques}]


%%%%%%%%%%%%%%%%%%%%%% TOCS %%%%%%%%%%%%%%%%%%%%%%
\printglossaries
\listoffigures
\listoftables
\tableofcontents

\end{document}